...
const IconSun = (props) => /* @__PURE__ */ jsx(Svg$1, { ...props, children: /* @__PURE__ */ jsxs("g", { children: [
...
const IconMoon = (props) => /* @__PURE__ */ jsx(Svg$1, { ...props, children: /* @__PURE__ */ jsx("path", { d: "M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z" }) });
...
const IconChevronDown = (props) => /* @__PURE__ */ jsx(Svg$1, { ...props, children: /* @__PURE__ */ jsx("path", { d: "M6 9l6 6 6-6" }) });
  ...
  return /* @__PURE__ */ jsxs("footer", { className: "app__footer", children: [
    /* @__PURE__ */ jsxs("div", { className: "row", children: [
      ...
      /* @__PURE__ */ jsxs("a", { href: MAIN_SITE.url, target: "_blank", rel: "noopener noreferrer", className: "footer__site-link", children: [
        /* @__PURE__ */ jsxs("span", { className: "pi-label", children: [
          "Школа ",
...
function PlanPage() {
  ...
  const CONSENT2 = "Я согласен на автоматические списания согласно условиям оферты";
  const loadOrders = () => {
    ...
    try {
      const answer = await api("/billing/checkout", {
        ...
        body: {
          ...
          consent: renew ? `${CONSENT2}. Списание раз в ${period.days} дн., ${price} ₽.` : null
        }
      });
  ...
  const toggleRenew = async (value) => {
    try {
      await api("/billing/auto-renew", {
        method: "POST",
        body: { value, consent: value ? CONSENT2 : null }
      });
      load();
    } catch (reason) {
      ...
      mine.kind === "paid" ? /* @__PURE__ */ jsxs("section", { className: "card", children: [
        ...
        mine.canAutoRenew || mine.autoRenew ? /* @__PURE__ */ jsxs(Fragment, { children: [
          /* @__PURE__ */ jsxs("div", { className: "check", children: [
            ...
            /* @__PURE__ */ jsxs("label", { htmlFor: "autoRenew", children: [
      ...
      mine.kind === "admin" ? null : /* @__PURE__ */ jsxs("section", { className: "card", children: [
        ...
        plans.length === 0 ? /* @__PURE__ */ jsx("p", { className: "text-muted", children: "Загружаем тарифы…" }) : /* @__PURE__ */ jsxs(Fragment, { children: [
          ...
          /* @__PURE__ */ jsxs("div", { className: "check", style: { marginTop: "var(--sp-3)" }, children: [
            ...
            /* @__PURE__ */ jsxs("label", { htmlFor: "renewOnBuy", children: [
              "Я согласен на автоматические списания согласно условиям",
              " ",
              /* @__PURE__ */ jsx(Link, { to: "/legal/offer", target: "_blank", rel: "noreferrer", children: "оферты" })
            ] })
...
const OFFER = {
  title: "Публичная оферта о предоставлении права использования сервиса «Доска Пи» на условиях подписки",
  ...
  sections: [
    {
      ...
      blocks: [
        p("1.1. В настоящей Публичной оферте содержатся условия договора о предоставлении права использования сервиса «Доска Пи» на условиях подписки (далее — «Договор»)."),
    ...
    },
    {
      ...
      blocks: [
        p(`Сервис — программа для ЭВМ «Доска Пи», доступ к которой предоставляется через сеть «Интернет» по адресу ${COMPANY.site}, предназначенная для совместной работы на виртуальной доске: рисования, размещения текста, фигур и изображений.`),
    ...
    },
    {
      ...
      blocks: [
        ...
        p("8.6. Не позднее чем за 3 (трое) суток до списания Исполнитель направляет Пользователю на адрес электронной почты уведомление с указанием суммы, даты списания и способа отключить автоматическое продление. Списание без такого уведомления не производится."),
        p("8.7. Цена следующего срока может измениться только с уведомлением, направляемым в порядке пункта 8.6 не позднее чем за 3 (трое) суток до списания и с указанием новой цены. Пользователь вправе отключить автоматическое продление до даты списания; уже оплаченные сроки изменение цены не затрагивает."),
        p("8.8. Согласие на автоматические списания даётся отдельным действием — отметкой в поле, которое по умолчанию не отмечено. Дата, время и текст согласия, а также его отзыв сохраняются Исполнителем."),
        p("8.9. Включить автоматическое продление для уже оплаченной Подписки, при оплате которой оно не было выбрано, технически невозможно: платёжный сервис допускает повторные списания только по счёту, помеченному соответствующим образом в момент оплаты. Автоматическое продление может быть выбрано при следующей оплате.")
      ]
    },
    {
  ...
  sections: [
    {
      ...
      blocks: [
        p("Доска Пи — онлайн-доска для совместной работы в браузере. Сервисом владеет и управляет Оператор, сведения о котором приведены в конце документа."),
  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        ...
        p("Кроме того, браузер получает несколько кук счётчика Яндекс.Метрики — он обезличенно фиксирует посещаемость и переходы по сайту. Эти куки, как и cookie_consent, отнесены к необходимым и ставятся независимо от выбора в баннере; сам выбор влияет только на то, показывается ли баннер повторно.")
      ]
    },
    {
      ...
      blocks: [
        p("Для работы отдельных функций используются сервисы сторонних организаций. Данные передаются им только в объёме, необходимом для работы соответствующей функции."),
        {
          ...
          rows: [
            ...
            ["Яндекс.Метрика", "обезличенные сведения о посещениях: страницы, переходы, действия на странице — для статистики посещаемости. Считается необходимой технической статистикой и подключается всегда, независимо от выбора в баннере cookie; запись действий на странице (Вебвизор) выключена на доске и странице входа по приглашению. Обработка — в Российской Федерации"]
          ]
        },
  ...
  sections: [
    {
      ...
      blocks: [
        p(`Оператору персональных данных — ${COMPANY.name}, ИНН ${COMPANY.inn}, ${COMPANY.status}, адрес электронной почты ${COMPANY.email} (далее — «Оператор»).`),
        DETAILS
      ]
    },
    {
      ...
      blocks: [
        ...
        list(
          ...
          "имя, указанное в учётной записи или при входе на доску по ссылке;",
    ...
    {
      title: "4. Какие действия разрешаются",
      blocks: [
        p("Сбор, запись, систематизация, накопление, хранение, уточнение (обновление, изменение), извлечение, использование, передача (предоставление, доступ), блокирование, удаление и уничтожение — как с использованием средств автоматизации, так и без них."),
        p("Передача третьим лицам допускается только в объёме, необходимом для оказания услуги: платёжному сервису — для проведения оплаты и формирования кассового чека, поставщику услуг связи и хостинга — для доставки писем и работы сервиса. Продажа, обмен и передача персональных данных в рекламных целях не производятся.")
      ]
    },
    {
      title: "5. Сколько хранится",
      blocks: [
        p("До удаления учётной записи, а в части сведений о платежах и согласиях — в течение сроков, установленных законодательством для документов бухгалтерского и налогового учёта. По истечении срока данные уничтожаются.")
      ]
    },
    {
      title: "6. Как отозвать",
      blocks: [
        p(`Согласие отзывается письмом на ${COMPANY.email} с адреса электронной почты, указанного в учётной записи, либо удалением учётной записи в личном кабинете.`),
        p("Отзыв согласия влечёт прекращение обработки и удаление персональных данных, кроме тех, хранение которых обязательно в силу закона, и делает невозможным дальнейшее пользование сервисом: учётная запись без адреса электронной почты не существует."),
        p("Отзыв согласия на обработку персональных данных не является отказом от подписки и не влечёт возврата вознаграждения; порядок возврата установлен Офертой.")
      ]
    }
  ]
};
...
function CookieBanner() {
  ...
  return /* @__PURE__ */ jsxs("div", { className: "cookie-banner", role: "region", "aria-label": "Согласие на куки", children: [
    /* @__PURE__ */ jsxs("p", { className: "cookie-banner__text", children: [
      "Сайт использует куки для корректной работы и обезличенную статистику посещений (Яндекс.Метрика). Подробнее — в",
      " ",
